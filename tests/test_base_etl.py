...
class TestBaseETL(unittest.TestCase):
    def setUp(self):
        self.config = {
            'DB_FILE': ':memory:',
            'CSV_FILE': 'sample_data_15_01_2022.csv'
        }
        self.base_etl = BaseETL(self.config)

    def test_initialization(self):
        self.assertEqual(self.base_etl.config['DB_FILE'], ':memory:')
        self.assertEqual(self.base_etl.config['CSV_FILE'], 'sample_data_15_01_2022.csv')
